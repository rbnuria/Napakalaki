...
public class PruebaNapakalaki {
    ...
    for(Monster it : monstruos){
        if(it.getBadConsequence().getLevels() != -1 && it.getBadConsequence().getnHiddenTreasures() == -1 &&
                it.getBadConsequence().getnVisibleTreasures() == -1 && it.getBadConsequence().getSpecificHiddenTreasures().isEmpty()
                && it.getBadConsequence().getSpecificVisibleTreasures().isEmpty()){
            aImprimir += it.toString() + "\n";
        }
    }
    System.out.println(aImprimir);
